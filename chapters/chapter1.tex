...
Hence, the Axiom of Replacement is pointless since, I mean, when does anyone used that? And what can we say about the Axiom of Infinity, other than that it's utter bullshit. So you work in a weaker theory, which we will call FST (short for Finite Set Theory).

As we know, $R(\omega)=V_\omega\vDash\mathrm{FST}$, and $\omega\notin V_\omega$, so $\mathrm{FST}$ can't prove there are such things as infinite cardinals. Even more, we have that the existence of such cardinal implies the consistency of FST! So we could say the Axiom of Infinity is some kind of Large Cardinal Axiom, but for ultrafinitist. We will keep this analogy when talking about consistency hierarchy later, so you don't need to keep thinking like an ultrafinitist until there\footnote{fortunetaly, otherwise you wouldn't get past the first page}.

\section{Inaccessible Cardinals}

\subsection{Fixed Point Lemma for Normal Functions}

As we saw, a weakly inaccessible cardinal $\kappa$ is just a uncountable limit regular cardinal. One of its properties is that it's an aleph fixed point, meaning $\aleph_\kappa=\kappa$. However, in my cofinality notes I proved being an aleph fixed point is not too strong, since we can prove there are arbitrarily large aleph fixed points. As promised, I will show here a more general result.
...
Also, if $\kappa$ is already a $F$ fixed point, we can let $G$ be an enumeration of the $F$ fixed point, and then $\kappa$ would also be a $G$ fixed point. So for example, if you list all aleph fixed point, $\kappa$ would be as far in the ``line'' of such fixed points as its own size, and so on.
...
To be fair, the previous proof is not entirely correct, since we didn't justified why $\lambda$ being inaccessible \textit{in} $V_{\kappa_0}$ implies that it's in fact inaccessible. In order to justify that, we need to show the possible counterexamples for $\lambda$ being a cardinal, regular and strong limit, would also be ``seen'' by $V_{\kappa_0}$:
...
And what we can say about the least worldly cardinal? Surprisingly, we can prove the following.
...
\begin{corollary}{}{inaccessible_wo_regular_worldly}
    In $\zfc+\mathrm{GCH}$ there are inaccessible cardinals with no regular worldly cardinals below it.
\end{corollary}
